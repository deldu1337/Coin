	...
	RoomFrame(int num) {
		...
		start_button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if(Integer.parseInt(dao.getUsingCoin(num))==0) {
					JOptionPane.showMessageDialog(null, "사용 중인 곡이 없습니다.", "경고", JOptionPane.WARNING_MESSAGE);
					return;
				}
		...
		use_button.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				...
				if(Integer.parseInt(dao.getRemain(num))==0) {
					...
					if(Integer.parseInt(remainCoin_field.getText())-Integer.parseInt(coin_field.getText())<= -1) {
						JOptionPane.showMessageDialog(null, "잔여 곡이 없습니다.", "경고", JOptionPane.WARNING_MESSAGE);
					}
					else {
