...
public class SecondFrame extends JFrame implements ActionListener {
	...
	ImageIcon img;
	JButton[] button_img = new JButton[21]; // 21크기 배열의 버튼 변수 선언 및 생성
	JButton admin_button; // 비상구 및 관리자 모드 버튼 변수 선언
	DAO dao;
	DTO dto;
	
	ImageIcon img1 = new ImageIcon("Image/StandardSeat_off.png"); // 버튼 기본 이미지
	ImageIcon img2 = new ImageIcon("Image/StandardSeat_on.png"); // 버튼에 마우스가 올라갈 때 이미지 이미지
	ImageIcon img3 = new ImageIcon("Image/FamillySeat_off.png"); // 버튼 기본 이미지
	ImageIcon img4 = new ImageIcon("Image/FamillySeat_on.png"); // 버튼에 마우스가 올라갈 때 이미지 이미지
	ImageIcon img5 = new ImageIcon("Image/Exit.png"); // 비상구 이미지

	SecondFrame() {
		img = new ImageIcon("Image/SecondFrame.png"); // 배경 이미지 생성
		...
		admin_button = new JButton("", img5); // 관리자 모드 버튼에 비상구 이미지 삽입
		...
		for (int i = 0; i < 7; i++) { // 총 7가지 경우를 수행하는 큰 반복문
			...
			for (int j = cnt; j < cnt + num; j++) { // 각각 4,2,2,2,2,4,5번으로 나뉘는, 총 21번의 반복을 수행하기 위한 작은 반복문
				if (i == 6) { // 17~21번 방(패밀리 룸)의 구현을 위한 조건문
					...
					button_img[j].setRolloverIcon(img4); // 버튼에 마우스가 올라갈 때 이미지 변환
					button_img[j].setBounds(x, y, 200, 100);
		...
		for (int i = 0; i < 21; i++)
			button_img[i].addActionListener(this); // 액션 메소드에 버튼 적용
